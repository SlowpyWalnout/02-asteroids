...
class Ship {
  ...
  tryShoot() {
    ...
    if (this.tripleTimer > 0) {
      const SPREAD = 0.22; // rad entre cada bala del abanico
      return [
...
const DROP_CHANCE     = 0.10; // prob. de soltar power-up por asteroide destruido
const POWERUP_TTL     = 10;   // segundos que el pickup flota antes de desaparecer
const TRIPLE_DURATION = 10;   // segundos que dura el disparo triple activo
...
class PowerUp {
  ...
  draw() {
    // Parpadeo en los últimos 3 segundos antes de desaparecer
    ...
    // Icono: tres rayitas en abanico (alude al disparo triple)
    ctx.beginPath();
...
function drawHUD() {
  ...
  if (ship.tripleTimer > 0) {
    ...
    ctx.fillText(`TRIPLE ${ship.tripleTimer.toFixed(1)}s`, W / 2, 46);
  }
}
